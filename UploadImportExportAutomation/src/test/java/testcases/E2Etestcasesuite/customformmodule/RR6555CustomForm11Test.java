...
public class RR6555CustomForm11Test extends TestBase {
	...
	public void rR6555CustomForm11Test(Hashtable<String, String> data) throws IOException, InterruptedException {
		...
		// MULTIPLE TYPE'S CUSTOM FORMS ON THE PROPERTY SUMMARY DASHBOARD WITH MULTIPLE
		// PROPERTIES.
		title("VERIFY THE IMPORT/EXPORT FEATURE OF THE RISK SCORE OF THE RANDOMLY CREATED MULTIPLE TYPE'S CUSTOM FORMS ON THE PROPERTY SUMMARY DASHBOARD WITH MULTIPLE PROPERTIES.");
		...
		try {
			...
			for (int j = count1 + 1; j > 1; j--) {
				...
				try {
					...
					if (riskscore.equals(score1_list.get(score1_list.size() - 1))) {
				...
				file.setCellData("Risk Scores", "Risk Status", j, updatescore1);
		...
		try {
			...
			try {
				String avgScore = (driver
						...
						.getText()).trim();
				int avgScore_actual = Integer.parseInt(avgScore);
				int totalUpdateScore = totalUpdateScore1 + totalScore2 + totalScore3;
