...
public class RR6555CustomForm8Test extends TestBase {

	@Test(dataProviderClass = TestUtil.class, dataProvider = "dp")
	public void rR6555CustomForm8Test(Hashtable<String, String> data) throws IOException, InterruptedException {

		execution(data, "rR6555CustomForm8Test");
		...
		// DASHBOARD WITH MULTIPLE PROPERTIES.
		title("VERIFY THE RISK SCORE OF THE ENVIRONMENTAL RECORD AND MULTIPLE TYPE CUSTOM FORMS WITH MULTIPLE DETAILS ON THE PROPERTY SUMMARY DASHBOARD AND PORTFOLIO DASHBOARD WITH MULTIPLE PROPERTIES.");
		...
		try {
			...
			try {
				...
				float env_score_float1 = Float.parseFloat(env_score1);
				float env_score_float2 = Float.parseFloat(env_score2);
				float env_score_float3 = Float.parseFloat(env_score3);
				String totalScore1_string = Integer.toString(totalScore1);
				float totalScore1_float1 = Float.parseFloat(totalScore1_string);
				String totalScore2_string = Integer.toString(totalScore2);
				float totalScore2_float1 = Float.parseFloat(totalScore2_string);
				String totalScore3_string = Integer.toString(totalScore3);
				float totalScore3_float1 = Float.parseFloat(totalScore3_string);

				float score_float1 = env_score_float1 + env_score_float2 + env_score_float3 + totalScore1_float1
						+ totalScore2_float1 + totalScore3_float1;
		...
		try {
			...
			try {
				...
				int env_score1_expected = Integer.parseInt(env_score1);
				int score_expected = env_score1_expected + totalScore1;
			...
			try {
				String Score = (driver
						...
						.getText()).trim();
				int avgScore_actual = Integer.parseInt(Score);
				float env_score2_float = Float.parseFloat(env_score2);
				float env_score3_float = Float.parseFloat(env_score3);
				String custom_totalScore2_String = Integer.toString(totalScore2);
				float custom_totalScore2_float = Float.parseFloat(custom_totalScore2_String);
				String custom_totalScore3_String = Integer.toString(totalScore3);
				float custom_totalScore3_float = Float.parseFloat(custom_totalScore3_String);
				float score_float1 = env_score2_float + env_score3_float + custom_totalScore2_float
						+ custom_totalScore3_float;
		...
		// UPDATE THE RISK SCORE OF THE ENVIRONMENTAL RECORD OF PROPERTY ONE
		title("UPDATE THE RISK SCORE OF THE ENVIRONMENTAL RECORD OF PROPERTY ONE");

		String env_updatescore1 = RandomStringUtils.randomNumeric(2);
		...
		try {
			...
			// enter the score in the score field
			type("envreports_asbestosmaterials_scoretxt_XPATH", env_updatescore1);
			...
			// property one
			try {
				String envrecord11 = "//span[text()='" + location_description1 + "']";
				String envrecord11_actual = (driver.findElement(By.xpath(envrecord11)).getText()).trim();

				if (envrecord11_actual.equals(location_description1)) {
					successMessage("The newly created environmental record in property one is displayed successfully.");
				} else {
					verificationFailedMessage(
							"The newly created environmental record in property one is not displayed.");
		...
		// UPDATE THE RISK SCORE OF THE ENVIRONMENTAL RECORD FOR PROPERTY TWO IS NOT
		// DONE INTENTIALLY.

		// UPDATE THE RISK SCORE OF THE ENVIRONMENTAL RECORD OF PROPERTY THREE
		title("UPDATE THE RISK SCORE OF THE ENVIRONMENTAL RECORD OF PROPERTY THREE");

		String env_updatescore3 = RandomStringUtils.randomNumeric(2);
		...
		try {
			...
			// click on the environmental record of property three
			String envrecord3 = "//span[text()='" + location_description3 + "']";
			driver.findElement(By.xpath(envrecord3)).click();
			consoleMessage("Clicked on the newly created environmental record.");
			...
			// enter the score in the score field
			type("envreports_asbestosmaterials_scoretxt_XPATH", env_updatescore3);
			...
			// property one
			try {
				String envrecord13 = "//span[text()='" + location_description3 + "']";
				String envrecord13_actual = (driver.findElement(By.xpath(envrecord13)).getText()).trim();

				if (envrecord13_actual.equals(location_description3)) {
					successMessage("The newly created environmental record in property one is displayed successfully.");
				} else {
					verificationFailedMessage(
							"The newly created environmental record in property one is not displayed.");
		...
		try {
			...
			for (int k = 1; k <= count1; k++) {
				...
				// enter the score in the score field
				customform_updatescore1 = RandomStringUtils.randomNumeric(2);
				type("customform_scoretxt_XPATH", customform_updatescore1);

				// collect the random details of the risk score
				customform_updatescore1_list.add(customform_updatescore1);
				consoleMessage("New count of data of the risk score : " + customform_updatescore1_list.size());
				...
				int updatescore1_int = Integer.parseInt(customform_updatescore1);
			...
			consoleMessage("While Update, End of for loop and The total number of count is "
					+ customform_updatescore1_list.size());
		...
		// UPDATE THE RISK SCORE DETAILS OF THE RANDOMLY CREATED CUSTOM FORM FROM THE
		// PORTFOLIO SUMMARY DASHBOARD FOR PROPERTY TWO
		title("UPDATE THE DETAILS OF THE FIELDS AND PAGES RANDOMLY FROM THE PORTFOLIO SUMMARY DASHBOARD FOR PROPERTY TWO");
		...
		try {
			...
			consoleMessage("While Update, The details in fields will entered " + count2 + " times.");
			...
			for (int m = 1; m <= count2; m++) {
				...
				String ele1 = "(//td[text()='[DND] AAAA Risk Test Prop 02'])[" + m + "]";
				driver.findElement(By.xpath(ele1)).click();
				consoleMessage("click on the property two i.e. " + m);
				...
				// enter the score in the score field
				customform_updatescore2 = RandomStringUtils.randomNumeric(2);
				type("customform_scoretxt_XPATH", customform_updatescore2);

				// collect the random details of the risk score
				customform_updatescore2_list.add(customform_updatescore2);
				consoleMessage("New count of data of the risk score : " + customform_updatescore2_list.size());
				...
				int updatescore2_int = Integer.parseInt(customform_updatescore2);
				totalUpdateScore2 = totalUpdateScore2 + updatescore2_int;
			...
			consoleMessage("While Update, End of for loop and The total number of count is "
					+ customform_updatescore2_list.size());
		...
		try {
			...
			try {
				...
				float env_score_float2 = Float.parseFloat(env_score2);
				float env_updatescore_float3 = Float.parseFloat(env_updatescore3);
		...
		try {
			...
			try {
				String Score = (driver
						...
						.getText()).trim();
				int avgScore_actual = Integer.parseInt(Score);
				int env_updatescore1_expected = Integer.parseInt(env_updatescore1);
				int score_expected = env_updatescore1_expected + totalUpdateScore1;
			...
			try {
				...
				int avgScore_actual = Integer.parseInt(Score);
				float env_score2_float = Float.parseFloat(env_score2);
				float env_updatescore3_float = Float.parseFloat(env_updatescore3);
				String custom_totalUpdateScore2_String = Integer.toString(totalUpdateScore2);
				float custom_totalUpdateScore2_float = Float.parseFloat(custom_totalUpdateScore2_String);
				String custom_totalScore3_String = Integer.toString(totalScore3);
				float custom_totalScore3_float = Float.parseFloat(custom_totalScore3_String);
				float score_float1 = env_score2_float + env_updatescore3_float + custom_totalUpdateScore2_float
						+ custom_totalScore3_float;
				float score_float2 = score_float1 / 3f;
				int score_expected = Math.round(score_float2);
