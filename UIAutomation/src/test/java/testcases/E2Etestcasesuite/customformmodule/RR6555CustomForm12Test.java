...
public class RR6555CustomForm12Test extends TestBase {
	...
	public void rR6555CustomForm12Test(Hashtable<String, String> data) throws IOException, InterruptedException {
		...
		// VALIDATE THE �VIEW RISK SCORING� PERMISSION OF THE SECURITY TAB AND
		// SECURITY SETTINGS FOR THE SINGLE TYPE CUSTOM FORM.
		title("VALIDATE THE �VIEW RISK SCORING� PERMISSION OF THE SECURITY TAB AND SECURITY SETTINGS FOR THE SINGLE TYPE CUSTOM FORM.");
		...
		try {
			...
			// ADD THE SECURITY PERMISSION OF VIEW RISK SCORE FOR THE JINESH (PROPERTY
			// MANAGER)
			title("ADD THE SECURITY PERMISSION OF VIEW RISK SCORE FOR THE JINESH (PROPERTY MANAGER)");

			// click on the security tab
			click("customform_securitytab_XPATH");

			// navigate to the view risk score field
			scrollTillElement("customform_securitytab_security13_XPATH");

			// click on the User to view risk score field
			click("customform_securitytab_security13_XPATH");

			// click on the none button
			click("customform_securitytab_nonebtn_XPATH");

			// enter the user name in the search field
			type("customform_securitytab_searchtxt_XPATH", data.get("username"));
		...
		try {
			...
			try {
				...
				try {
					...
					if (riskCard == true) {
						verificationFailedMessage(
								"The risk card is displayed in the proprty dashboard without the view permission in the admin user.");
					} else {
						successMessage(
								"The risk card is not displayed in the proprty dashboard as expected in the admin user.");
					}
				} catch (Throwable t) {
					successMessage(
							"The risk card is not displayed in the proprty dashboard as expected in the admin user.");
