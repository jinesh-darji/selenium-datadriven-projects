...
public class RR6555CustomForm5Test extends TestBase {
	...
	public void rR6555CustomForm5Test(Hashtable<String, String> data) throws IOException, InterruptedException {
		...
		// ADD THE DETAILS IN THE MULTIPLE TYPE CUSTOM FORM RANDOMLY WITH THE RISK SCORE
		...
		try {
			...
			// validate the risk score of the multiple type custom form of the property
			try {
				String Score2 = "//td[text()='" + mname + "']//following-sibling::td[@class='ng-binding']";
				String riskScore = (driver.findElement(By.xpath(Score2)).getText()).trim();
				int riskScore_actual = Integer.parseInt(riskScore);

				if (riskScore_actual == mtotalScore) {
					successMessage(
							"The risk score of the multiple type custom form of the property is displayed correctly.");
				} else {
					verificationFailedMessage(
							"The risk score of the multiple type custom form of the property is not displayed correctly.");
				}
			} catch (Throwable t) {
				verificationFailedMessage(
						"The risk score of the multiple type custom form of the property is not displayed correctly.");
		...
		try {
			...
			try {
				...
				String riskScore = (driver.findElement(By.xpath(Score1)).getText()).trim();
				int riskScore_actual = Integer.parseInt(riskScore);
				int score_expected = Integer.parseInt(supdatescore);
			...
			try {
				...
				if (riskScore_actual == mtotalUpdateScore) {
					successMessage(
							"The risk score of the multiple type custom form of the property is displayed correctly.");
				} else {
					verificationFailedMessage(
		...
		try {
			...
			try {
				...
				if (propertyName == true) {
					...
					// validate the risk score of the property
					try {
						String Score = "//td[text()='" + mname + "']";
						boolean riskScore = driver.findElement(By.xpath(Score)).isDisplayed();

						if (riskScore == true) {
							verificationFailedMessage(
									"The risk score of the multiple type custom form of the property is displayed.");
						} else {
							successMessage(
									"The risk score of the multiple type custom form of the property is not displayed as expected.");
						}
					} catch (Throwable t) {
						successMessage(
								"The risk score of the multiple type custom form of the property is not displayed as expected.");
