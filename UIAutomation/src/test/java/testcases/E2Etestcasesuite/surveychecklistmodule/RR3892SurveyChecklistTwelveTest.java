...
public class RR3892SurveyChecklistTwelveTest extends TestBase {

	@Test(dataProviderClass = TestUtil.class, dataProvider = "dp")
	public void rR3892SurveyChecklistTwelveTest(Hashtable<String, String> data)
			throws IOException, InterruptedException {

		execution(data, "rR3892SurveyChecklistTwelveTest");

		// AS A SITE MANAGER: ALL TASKS REGARDLESS OF THE TASK STATUS "INPROGRESS"
		// SHOULD GET DELETED OR NOT ON THE PARENT QUESTION-ANSWER SWITCH - The "In
		// Progress" task consider as a Active task so it should Deleted.

		title("AS A PROPERTY MANAGER: TASKS ASSOCIATED WITH THE DEPENDENT QUESTION SHOULD BE DELETED WHEN ITS PARENT QUESTION IS SWITCHED - The \\\"INPROGRESS\\\" task consider as a Active task so it should Delete.");
		...
		try {
			...
			verifyTrue("questionnaire_createdrecordauto_12_XPATH", "The newly questionnaire is not created.");

			// CONFIGRATION OF QUESTION
			title("CONFIGRATION OF QUESTION");

			// click on the created questionnaire
			click("questionnaire_createdrecordauto_12_XPATH");
			...
			switchVerification("questionnaire_createdrecordauto_12_XPATH", "Test Auto Questionnaire Twelve Title",
					"The Test Auto Questionnaire Twelve Title record is not displayed in the questionnaire list.");
		...
		try {
			...
			try {
				...
				switchVerification("task_createdtask_12_XPATH", "Test Task Twelve Title",
						"The Test Task Twelve Title is not displayed.");

				// click on the auto generated task
				click("task_createdtask_12_XPATH");
			...
			try {
				...
				// add explicit wait on task screen
				explicitWait("survey_task_searchfield_CSS");
				...
				// verify the in progress task is displayed or not
				try {
					String str1 = driver.findElement(By.xpath(OR.getProperty("task_inprogress_filtered_XPATH")))
							.getText();
					if (str1.equals("In Progress")) {

						verificationFailedMessage("The inprogress task is not verified.");
