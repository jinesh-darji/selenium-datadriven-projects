...
public class RR3892SurveyChecklistSevenTest extends TestBase {

	@Test(dataProviderClass = TestUtil.class, dataProvider = "dp")
	public void rR3892SurveyChecklistSevenTest(Hashtable<String, String> data)
			throws IOException, InterruptedException {

		execution(data, "rR3892SurveyChecklistSevenTest");

		// ALL TASKS REGARDLESS OF THE TASK STATUS "CLOSE" SHOULD GET DELETED OR NOT ON
		// THE PARENT QUESTION-ANSWER SWITCH - The "Closed" task consider as a Inactive
		// task so it should be Deleted.

		title("ALL TASKS REGARDLESS OF THE TASK STATUS \\\"CLOSE\\\" SHOULD GET DELETED OR NOT ON THE PARENT QUESTION-ANSWER SWITCH - The \\\"Closed\\\" task consider as a Inactive task so it should be Deleted.");
		...
		try {
			...
			verifyTrue("questionnaire_createdrecordauto_7_XPATH", "The newly questionnaire is not created.");

			// CONFIGRATION OF QUESTION
			title("CONFIGRATION OF QUESTION");

			// click on the created questionnaire
			click("questionnaire_createdrecordauto_7_XPATH");
			...
			switchVerification("questionnaire_createdrecordauto_7_XPATH", "Test Auto Questionnaire Seven Title",
					"The Test Auto Questionnaire Seven Title record is not displayed in the questionnaire list.");
		...
		// VERIFY AUTO GENERATED TASK IS DISPLAYED OR NOT AND MAKE IT IN CLOSE STATUS
		title("VERIFY AUTO GENERATED TASK IS DISPLAYED OR NOT AND MAKE IT IN CLOSE STATUS");
		...
		try {
			...
			switchVerification("task_createdtask_7_XPATH", "Test Task Seven Title",
					"The Test Task Seven Title is not displayed.");

			// click on the auto generated task
			click("task_createdtask_7_XPATH");
			...
			// wait for the element
			explicitWaitClickable("survey_task_backbtn_CSS");
			...
			// verify the closed button is displayed or not
			switchVerification("task_closedbtn_filtered_XPATH", "Closed", "The Closed task is not displayed.");
		...
		try {
			...
			// verify the closed button is displayed or not
			try {
				String str1 = driver.findElement(By.xpath(OR.getProperty("task_closedbtn_filtered_XPATH"))).getText();

				if (str1.equals("Closed")) {

					verificationFailedMessage("The closed task is not deleted from the closed task list.");

				} else {
					successMessage("The closed task is deleted and verified successfully.");
