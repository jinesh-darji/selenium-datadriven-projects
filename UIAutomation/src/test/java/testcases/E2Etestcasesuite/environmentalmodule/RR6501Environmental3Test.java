...
public class RR6501Environmental3Test extends TestBase {

	@Test(dataProviderClass = TestUtil.class, dataProvider = "dp")
	public void rR6501Environmental3Test(Hashtable<String, String> data) throws InterruptedException, IOException {

		execution(data, "rR6501Environmental3Test");
		...
		// KEEP ENVIRONMENTAL REPORT DATE FIELD REQUIRED IN THE ENVIRONMENTAL FORMS AND
		// OPTIONAL IN THE ENVIRONMENTAL REPORT SCREENS
		title("KEEP ENVIRONMENTAL REPORT DATE FIELD REQUIRED IN THE ENVIRONMENTAL FORMS AND AND OPTIONAL IN THE ENVIRONMENTAL REPORT SCREENS");
		...
		try {
			...
			try {
				...
				if (required == true) {
					...
					consoleMessage("The required checkbox is selected successfully.");

					// click on the required checkbox
					click("administration_environmental_environmentalreport_daterequiredckbx_XPATH");

					successMessage("The required checkbox is not selected as expected.");
		...
		try {
			...
			try {
				...
				if (afterOneY_actual.equals(afterOneY_string)) {
					verificationFailedMessage("The Next Assessment date is displayed default.");

				} else {
					successMessage("The Next Assessment date is not displayed default as expected.");
				}
		...
		try {
			...
			try {
				...
				if (reddot == true) {
					successMessage("The red dot is displayed successfully for the newly created environmental report.");

				} else {
					verificationFailedMessage(
							"The red dot is not displayed for the newly created environmental report.");
				}
			} catch (Throwable t) {
				verificationFailedMessage("The red dot is not displayed for the newly created environmental report.");
			}
			...
			try {
				...
				if (reportDate_actual.equals(today_string)) {
					verificationFailedMessage(
							"The report date of the newly created environmental report is displayed.");

				} else {
					successMessage(
							"The report date of the newly created environmental report is not displayed as expected.");
				}
			} catch (Throwable t) {
				successMessage(
						"The report date of the newly created environmental report is not displayed as expected.");
			...
			try {
				...
				if (reportDate_actual.equals(afterOneY_string)) {
					verificationFailedMessage(
							"The Next Assessment date of the newly created environmental report is displayed.");

				} else {
					successMessage(
							"The Next Assessment date of the newly created environmental report is not displayed as expected.");
				}
			} catch (Throwable t) {
				successMessage(
						"The Next Assessment date of the newly created environmental report is not displayed as expected.");
				...
				for (int i = 1; i < reportCount; i++) {
					...
					if (environmentalTitle.equals(title1)) {
						...
						try {
							...
							String reddot = "//p[text()='" + environmentalTitle
									+ "']//parent::td//preceding-sibling::td[@class='ng-scope']//span[@style='background-color:#d43f3a']";
							boolean reddot_actual = driver.findElement(By.xpath(reddot)).isDisplayed();

							if (reddot_actual == true) {
								successMessage(
										"The red dot is displayed successfully for the newly created environmental report.");
