...
public class RR6508Environmental1Test extends TestBase {
	...
	public void rR6508Environmental1Test(Hashtable<String, String> data) throws InterruptedException, IOException {
		...
		String labelField_update = RandomStringUtils.randomAlphabetic(8);
		...
		// UPDATE THE NEW FIELD IN THE ENVIRONEMNTAL REPORT FROM ENVIRONMENTAL FORM AND
		// VALIDATE ON THE INDIVIDUAL ENVIRONMENTAL REPORT
		title("UPDATE THE NEW FIELD IN THE ENVIRONEMNTAL REPORT FROM ENVIRONMENTAL FORM AND VALIDATE ON THE INDIVIDUAL ENVIRONMENTAL REPORT");
		...
		try {
			...
			String fieldLabel = "//span[text()='" + labelField + " (" + nameField
					+ ")']//parent::div[@class='col-md-6']//following-sibling::div[@class='col-md-2 pull-right']//button[@ng-click='editSection(section)']";
			driver.findElement(By.xpath(fieldLabel)).click();
			consoleMessage("Clicked on the newly created field.");

			// update the details in the label field
			type("administration_environmental_environmentalreport_labeltxt_XPATH", labelField_update);
			...
			// validate the newly created field is displayed or not
			String fieldLabelUpdate = "//span[text()='" + labelField_update + " (" + nameField + ")']";
			boolean fieldLabelUpdate_actual = driver.findElement(By.xpath(fieldLabelUpdate)).isDisplayed();

			if (fieldLabelUpdate_actual == true) {

				successMessage("The updated environmental report field is displayed successfully.");

			} else {

				verificationFailedMessage("The updated environmental report field is not displayed.");
			}
			...
			// click on the edit button of the updated field
			String editButton = "//span[text()='" + labelField_update
					+ "']//following-sibling::button[@class='btn btn-default btn-sm pull-right']";
			driver.findElement(By.xpath(editButton)).click();
			consoleMessage("Clicked on the edit button of the newly created field.");

			// validate the name of the updated field
			try {
				String fieldName = "//span[text()='" + labelField_update
						+ "']//following-sibling::div//input[@ng-model='item.section']";
				String fieldName_actual = driver.findElement(By.xpath(fieldName)).getAttribute("value");

				if (fieldName_actual.equals(nameField)) {
					successMessage("The updated field name is displayed in the environmental report.");
				} else {
					verificationFailedMessage("The updated field name is not displayed in the environmental report.");
				}
			...
			// validate the label of the updated field
			try {
				String fieldLabel1 = "//span[text()='" + labelField_update
						+ "']//following-sibling::div//input[@ng-model='item.label']";
				String fieldLabel1_actual = driver.findElement(By.xpath(fieldLabel1)).getAttribute("value");

				if (fieldLabel1_actual.equals(labelField_update)) {
					successMessage("The updated field label is displayed in the environmental report.");
				} else {
					verificationFailedMessage("The updated field label is not displayed in the environmental report.");
				}
			...
			// validate the enabled checkbox of the updated field
			try {
				String enabledckbx = "//span[text()='" + labelField_update
						+ "']//following-sibling::div[@class='col-md-12 in']//input[@ng-model='item.enabled']";
				boolean enabledckbx_actual = driver.findElement(By.xpath(enabledckbx)).isSelected();

				if (enabledckbx_actual == true) {
					successMessage(
							"The enabled checkbox is displayed selected for the updated field in the environmental report.");
				} else {
					verificationFailedMessage(
							"The enabled checkbox is not displayed selected for the updated field in the environmental report.");
				}
			...
			// validate the required checkbox of the updated field
			try {
				String requiredckbx = "//span[text()='" + labelField_update
						+ "']//following-sibling::div[@class='col-md-12 in']//input[@ng-model='item.required']";
				boolean requiredckbx_actual = driver.findElement(By.xpath(requiredckbx)).isSelected();

				if (requiredckbx_actual == true) {
					successMessage(
							"The required checkbox is displayed selected for the updated field in the environmental report.");
				} else {
					verificationFailedMessage(
							"The required checkbox is not displayed selected for the updated field in the environmental report.");
				}
		...
		// VALIDATE THE UPDATED FIELD IN THE PREVIOUS RECORD OF THE ENVIRONMENTAL REPORT
		// AND CREATE NEW RECORD OF THE INDIVIDUAL ENVIRONMENTAL REPORT
		title("VALIDATE THE UPDATED FIELD IN THE PREVIOUS RECORD OF THE ENVIRONMENTAL REPORT AND CREATE NEW RECORD OF THE INDIVIDUAL ENVIRONMENTAL REPORT");

		try {
			// validate the previously created environmental report record
			title("validate the previously created environmental report record");
			...
			// validate the newly added field is displayed or not
			try {
				String dropdownField = "//select[@id='select-" + labelField_update + "']";
				boolean dropdownField_actual = driver.findElement(By.xpath(dropdownField)).isDisplayed();
			...
			// select the option from the updated field
			String dropdownField = "//select[@id='select-" + labelField_update + "']";
			WebElement dropdown = driver.findElement(By.xpath(dropdownField));
			Select select = new Select(dropdown);
			...
			click("envreport_updatebtn_XPATH");
			...
			// validate the red color asterisk sign is displayed or not in newly added field
			try {
				String requiredIcon = "//label[contains(text(),'" + labelField_update
						+ "')]//span[@class='required-field ng-scope']";
				boolean requiredIcon_actual = driver.findElement(By.xpath(requiredIcon)).isDisplayed();
			...
			// validate the newly added field is displayed or not
			try {
				String dropdownField1 = "//select[@id='select-" + labelField_update + "']";
				boolean dropdownField1_actual = driver.findElement(By.xpath(dropdownField1)).isDisplayed();

				if (dropdownField1_actual == true) {
					successMessage("The dropdown field is displayed successfully.");
				} else {
					verificationFailedMessage("The dropdown field is not displayed.");
				}
			...
			// enter the title
			type("envreporttitletxt_CSS", title2);
			...
			// select the option from the newly added field
			String dropdownField3 = "//select[@id='select-" + labelField_update + "']";
			WebElement dropdown3 = driver.findElement(By.xpath(dropdownField3));
			...
			// enter the newly created environmental report in the search field
			type("task_listofchecklist_filterfield_CSS", title2);

			// verification of the environmental report is added or not
			try {
				String record2 = "//span[text()='" + title2 + "']";
				String record2_actual = (driver.findElement(By.xpath(record2)).getText()).trim();

				if (record2_actual.equals(title2)) {
		...
		// DELETE THE UPDATED FIELD FROM THE ENVIRONMENTAL REPORT, WHEN ENVIRONMENTAL
		// REPORT RECORDS ARE CREATED WITH THE RESPECTIVE FIELD
		title("DELETE THE UPDATED FIELD FROM THE ENVIRONMENTAL REPORT, WHEN ENVIRONMENTAL REPORT RECORDS ARE CREATED WITH THE RESPECTIVE FIELD");
		...
		try {
			...
			// click on the newly created field
			String fieldLabel = "//span[text()='" + labelField_update + " (" + nameField
		...
		try {
			...
			try {
				String deletedRecord = "//span[text()='" + labelField_update + "']";
