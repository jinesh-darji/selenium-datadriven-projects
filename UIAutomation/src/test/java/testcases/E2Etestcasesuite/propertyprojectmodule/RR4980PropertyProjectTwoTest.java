...
public class RR4980PropertyProjectTwoTest extends TestBase {

	@Test(dataProviderClass = TestUtil.class, dataProvider = "dp")
	public void rR4980PropertyProjectTwoTest(Hashtable<String, String> data) throws IOException, InterruptedException {

		execution(data, "rR4980PropertyProjectTwoTest");

		// THE "ASSIGNED TO" FIELD MUST FETCH THE DEFAULT USER LIST AS CONFIGURED IN
		// SECURITY SETTINGS - CAPEX PROJECTS.

		System.out.println(
				"THE \"ASSIGNED TO\" FIELD MUST FETCH THE DEFAULT USER LIST AS CONFIGURED IN SECURITY SETTINGS - CAPEX PROJECTS.");
		...
		try {
			...
			// APPLY THE ACCESS OF DEFAULT PROPERTY PROJECT CAPEX PROJECTS TASK
			// ASSIGNEES

			helper.addPermissionOneUsersRoleTask(data, "search_ss", "propertyproject_mu_task_searchedss1_XPATH");

			// ADD SECURITY SETTINGS OF VIEW CAPEX PROJECTS TO USER/ROLE ACCESS
			...
			Reporter.log(
					"*************** ADD SECURITY SETTINGS OF VIEW CAPEX PROJECTS TO USER/ROLE ACCESS *************");
			log.info("*************** ADD SECURITY SETTINGS OF VIEW CAPEX PROJECTS TO USER/ROLE ACCESS *************");

			// APPLY THE VIEW CAPEX PROJECTS PERMISSION

			helper.addPermissionOneUsersRoleTask(data, "security_setting_5", "propertyproject_securitysettings5_XPATH");

			// APPLY THE ADD CAPEX PROJECTS PERMISSION

			helper.addPermissionOneUsersRoleTask(data, "security_setting_6", "propertyproject_securitysettings6_XPATH");
			...
			type("propertyproject_le_filtertxt_CSS", data.get("security_setting_9"));
			ngDriver.waitForAngularRequestsToFinish();
		...
		System.out.println(
				"**************** VERIFY DEFAULT USERS / ROLES DISPLAYED IN THE ASSIGN TO FIELD - CAPEX PROJECTS ****************");
		test.log(LogStatus.INFO,
				"**************** VERIFY DEFAULT USERS / ROLES DISPLAYED IN THE ASSIGN TO FIELD - CAPEX PROJECTS ****************");
		Reporter.log(
				"**************** VERIFY DEFAULT USERS / ROLES DISPLAYED IN THE ASSIGN TO FIELD - CAPEX PROJECTS ****************");
		...
		try {
			...
			System.out.println("**************** DELETE CAPEX PROJECTSS ****************");
			test.log(LogStatus.INFO, "**************** DELETE CAPEX PROJECTSS ****************");
			Reporter.log("**************** DELETE CAPEX PROJECTSS ****************");
			log.info("**************** DELETE CAPEX PROJECTSS ****************");
			...
			// click on the delete button of the newly created CAPEX PROJECTSs
			...
			for (int i = 0; i < count4 - 1; i++) {
				...
				// delete the all the CAPEX PROJECTSs
				click("propertyproject_cp_deletebtn_CSS");
				System.out.println("Deleted the all the CAPEX PROJECTSs");
				ngDriver.waitForAngularRequestsToFinish();
		...
		try {
			...
			// RESET THE PERMISSION TO DEFAULT PROPERTY PROJECT CAPEX TASK ASSIGNEES

			helper.resetPermission(data, "search_ss", "propertyproject_mu_task_searchedss1_XPATH");

			// RESET THE PERMISSION TO TASK CREATOR

			helper.resetPermission(data, "security_setting_9", "propertyproject_securitysettings13_XPATH");
