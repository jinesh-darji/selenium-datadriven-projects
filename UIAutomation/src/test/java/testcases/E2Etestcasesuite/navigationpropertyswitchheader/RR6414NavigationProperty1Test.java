...
public class RR6414NavigationProperty1Test extends TestBase {

	@Test(dataProviderClass = TestUtil.class, dataProvider = "dp")
	public void rR6414NavigationProperty1Test(Hashtable<String, String> data) throws InterruptedException, IOException {

		execution(data, "rR6414NavigationProperty1Test");

		Helper helper = new Helper();

		// PROPERTY PROJECT MODULE - NAVIGATION SCENARIO OF THE OHS SUMMARY, PROPERTY
		// DETAILS, BUILDING DETAILS, OWNERS/INVESTORS, SITE TEAM, HISTORY, SURVEYS,
		// INSPECTIONS AND DOCUMENTS/REPORTS.
		title("PROPERTY PROJECT MODULE - NAVIGATION SCENARIO OF THE OHS SUMMARY, PROPERTY DETAILS, BUILDING DETAILS, "
		...
		try {
			...
			type("ohs_documentreport_titletxt_XPATH", data.get("ppdocument_title"));

			// select the type of the document
			select("propertyproject_documentreport_typedd_XPATH", data.get("ppdocument_type"));

			// click on the save button
			click("ohs_documentreport_savebtn_XPATH");

			// enter the newly created document name in the search field
			type("propertyproject_documentreport_searchtxt_XPATH", data.get("ppdocument_title"));

			// validate the newly created document report record
			try {
				String record = "//td[text()='" + data.get("ppdocument_title") + "']";
		...
		try {
			...
			type("propertyproject_mu_undertakingnumbertxt_CSS", title);
		...
		// THE PROPERTY PROJECT SUMMARY SCREEN
		title("VALIDATE THE NAVIGATION AFTER SWITCHING THE PROPERTY FROM THE HEADER OF THE PROPERTY PROJECT SUMMARY SCREEN");
		...
		try {
			...
			// validate the Mortgage Undertakings count
			switchVerification("propertyproject_summarycount_XPATH", "1",
					"The correct Mortgage Undertakings count is not displayed.");
			...
			// validate the Mortgage Undertakings count
			switchVerification("propertyproject_summarycount_XPATH", "0",
					"The correct Mortgage Undertakings count is not displayed.");
		...
		try {
			...
			try {
				...
				if (title_actual.equals(data.get("ppdocument_title"))) {
			...
			try {
				String titlelist = "//td[text()='" + data.get("ppdocument_title") + "']";
		...
		try {
			...
			// validate the newly created document report record
			try {
				String record1 = "//td[text()='" + data.get("ppdocument_title") + "']";
