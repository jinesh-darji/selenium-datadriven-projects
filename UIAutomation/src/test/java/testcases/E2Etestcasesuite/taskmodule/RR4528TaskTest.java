...
public class RR4528TaskTest extends TestBase {

	@Test(dataProviderClass = TestUtil.class, dataProvider = "dp")
	public void rR4528TaskTest(Hashtable<String, String> data) throws IOException, InterruptedException {

		execution(data, "rR4528TaskTest");

		// VERIFY THE TASK WITH TASK TAG PERMISSION (MODULE NAMES: ENVIRONMENT, ACCESS
		// TO SURVEY/CHECKLIST SECTION: USER-JINESH MISC 2 AND ROLE-MAINTENANCE)

		System.out.println(
				"VERIFY THE TASK WITH TASK TAG PERMISSION (MODULE NAMES: ENVIRONMENT, ACCESS TO SURVEY/CHECKLIST SECTION: USER-JINESH MISC 2 AND ROLE-MAINTENANCE).");
		...
		try {
			...
			try {
				...
				type("propertyproject_le_filtertxt_CSS", data.get("security_settings_2"));
				ngDriver.waitForAngularRequestsToFinish();
				System.out.println("Entered security setting name in the search field.");

				// click on the searched result
				click("securitysettings_surveychecklistsection_XPATH");
				ngDriver.waitForAngularRequestsToFinish();
		...
		try {
			...
			switchVerification("task_createdtask_tasktagpermission44_XPATH", "Test Task Tap Permission 4",
					"The Test Task Tap Permission 4 is not displayed.");
		...
		try {
			...
			try {
				...
				click("task_createdtask_tasktagpermission44_XPATH");
				ngDriver.waitForAngularRequestsToFinish();
			...
			try {
				...
				if (str4 == true) {
					...
					System.out.println("The Survey/Checklist tab is displayed as expected.");
					test.log(LogStatus.INFO, "The Survey/Checklist tab is displayed as expected.");
					Reporter.log("The Survey/Checklist tab is displayed as expected.");
					log.info("The Survey/Checklist tab is displayed as expected.");
			...
			try {
				...
				try {
					String str1 = driver
							.findElement(By.xpath(OR.getProperty("task_createdtask_tasktagpermission44_XPATH")))
							.getText();
					ngDriver.waitForAngularRequestsToFinish();
					if (str1.equals("Test Task Tap Permission 4")) {

						Assert.assertTrue(
								isElementPresent(
										By.xpath(OR.getProperty("task_createdtask_tasktagpermission44_XPATH"))),
								"The Test Task Tap Permission 4 is not displayed.");
		...
		try {
			...
			helper.deleteVerification("task_createdtask_tasktagpermission44_XPATH", "Test Task Tap Permission 4");
