...
public class RR5192SmartCenter2Test extends TestBase {
	...
	public void rR5192SmartCenter2Test(Hashtable<String, String> data) throws IOException, InterruptedException {
		...
		if (!(TestUtil.isTestRunnable("rR5192SmartCenter2Test", excel))) {

			throw new SkipException("Skipping the test " + "rR5192SmartCenter2Test".toUpperCase() + "as the Run mode is NO");
		}
		...
		try {
			...
			try {
				...
				ngDriver.waitForAngularRequestsToFinish();
				System.out.println("Entered contractor name in the search field.");

				// click on the searched role name
				click("task_permission_securitysettings_seachedrole_XPATH");
				ngDriver.waitForAngularRequestsToFinish();
				System.out.println("Clicked on the searched role name.");
			...
			System.out.println("**************** LOGIN IN WITH CONTRACTOR USER ****************");
			test.log(LogStatus.INFO, "**************** LOGIN IN WITH CONTRACTOR USER ****************");
			Reporter.log("**************** LOGIN IN WITH CONTRACTOR USER ****************");
			log.info("**************** LOGIN IN WITH CONTRACTOR USER ****************");
			...
			try {
				...
				Reporter.log(
						"**************** DELETE THE NEWLY CREATED TASK FROM THE ACTIVE TASK LIST ****************");
				log.info("**************** DELETE THE NEWLY CREATED TASK FROM THE ACTIVE TASK LIST ****************");
